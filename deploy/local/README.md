...
프록시를 거치는 이유는 CORS가 아니라 **쿠키**입니다. 세션은 `HttpOnly` 쿠키이고, :5173에서 :8080으로 직접 부르면 브라우저가 그걸 교차 출처로 보아 되돌려 보내지 않습니다 — 로그인이 성공한 것처럼 보이고 그 다음 요청이 전부 401이 됩니다. 프록시를 지나면 전부 한 출처입니다.

`/api`와 `/s` **둘 다** 필요합니다. 앞은 API 전부, 뒤는 공유 링크입니다. 나머지 경로는 Vite가 인터페이스를 직접 서빙합니다.
